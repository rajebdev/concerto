...
use quote::quote;
use syn::{parse_macro_input, Expr, ExprLit, ExprPath, ItemFn, ItemImpl, Lit, Meta, MetaNameValue};
...
/// async fn my_interval_task() {
///     println!("Runs every 30 milliseconds (default unit)");
/// }
/// 
/// // Using shorthand duration format
/// #[scheduled(fixed_rate = "5s")]
/// async fn five_seconds_task() {
///     println!("Runs every 5 seconds");
/// }
/// 
/// #[scheduled(fixed_rate = "10m")]
/// async fn ten_minutes_task() {
///     println!("Runs every 10 minutes");
/// }
/// 
/// #[scheduled(fixed_rate = "2h")]
/// async fn two_hours_task() {
///     println!("Runs every 2 hours");
/// }
/// 
/// // Using time_unit parameter
/// #[scheduled(fixed_rate = 5, time_unit = TimeUnit::Minutes)]
/// async fn five_minute_task() {
///     println!("Runs every 5 minutes");
/// }
/// 
/// #[scheduled(fixed_rate = "${app.interval}", time_unit = "${app.time_unit:milliseconds}")]
/// async fn config_based_task() {
///     println!("Interval from config");
/// }
/// 
/// // Zone parameter only works with cron expressions
/// #[scheduled(cron = "0 0 9 * * *", zone = "Asia/Jakarta")]
/// async fn jakarta_morning_task() {
///     println!("Runs at 9 AM Jakarta time");
/// }
/// 
/// #[scheduled(fixed_rate = 4, enabled = true)]
/// async fn enabled_task() {
///     println!("This task is always enabled");
/// }
/// ```
/// 
/// # Parameters
...
///   - Can be a number (uses `time_unit` parameter, default: milliseconds)
///   - Can be shorthand string: "5s", "10m", "2h", "1d", "500ms"
/// - `fixed_delay`: Fixed delay between task completions (same format as `fixed_rate`)
/// - `time_unit`: Time unit for numeric intervals (milliseconds, seconds, minutes, hours, days)
///   - Can use string: `"minutes"` or enum: `TimeUnit::Minutes`
///   - Default: milliseconds
///   - **Not applicable to cron expressions**
///   - Ignored if shorthand format is used (e.g., "5s")
/// - `zone`: Timezone for cron expressions (e.g., "Asia/Jakarta", "UTC")
///   - **Only works with `cron`**, ignored for `fixed_rate`/`fixed_delay`
///   - Default: "local"
/// - `initial_delay`: Delay before first execution (uses same time_unit)
/// - `enabled`: Enable/disable task (boolean or config placeholder)
///   - Can use literal: `true`/`false` or config: `"${app.enabled}"`
///   - Default: true
...
pub fn scheduled(args: TokenStream, input: TokenStream) -> TokenStream {
        ...
        match arg {
            Meta::NameValue(MetaNameValue { path, value, .. }) => {
                ...
                match path_str.as_str() {
                    ...
                    "zone" => {
                        zone = Some(match value {
                            Expr::Lit(ExprLit { lit: Lit::Str(s), .. }) => s.value(),
                            _ => panic!("zone must be a string (e.g., 'Asia/Jakarta', 'UTC')"),
                        });
                    }
                    _ => {}
                }
    ...
    let schedule_type = schedule_type.expect("Must specify schedule type (cron, fixed_rate, or fixed_delay)");
    let schedule_value = schedule_value.expect("Must specify schedule value");
...
/// Macro for implementing the Runnable trait with scheduling configuration
/// 
/// This macro must be used on an impl block for the Runnable trait.
/// The task will only run if it's registered via `SchedulerBuilder::runnable()`
/// 
/// # Examples
/// 
/// ```rust
...
/// # Parameters
/// 
/// Same as `#[scheduled]` macro:
/// - `cron`: Cron expression
/// - `fixed_rate`: Fixed interval between executions
/// - `fixed_delay`: Fixed delay between completions
/// - `time_unit`: Time unit (milliseconds, seconds, minutes, hours, days)
/// - `zone`: Timezone (only for cron)
/// - `initial_delay`: Delay before first execution
/// - `enabled`: Enable/disable task
#[proc_macro_attribute]
pub fn scheduled_impl(args: TokenStream, input: TokenStream) -> TokenStream {
    let attr_args = syn::parse_macro_input!(args with syn::punctuated::Punctuated::<Meta, syn::Token![,]>::parse_terminated);
    ...
    // Parse macro arguments (same as scheduled macro)
    for arg in attr_args {
        match arg {
            Meta::NameValue(MetaNameValue { path, value, .. }) => {
                let path_str = path
                ...
                match path_str.as_str() {
                    ...
                    "time_unit" => {
                        time_unit = Some(match value {
                            ...
                            _ => panic!("time_unit must be a string or TimeUnit::* constant"),
                        });
                    }
                    "zone" => {
                        zone = Some(match value {
                            Expr::Lit(ExprLit { lit: Lit::Str(s), .. }) => s.value(),
                            _ => panic!("zone must be a string"),
                        });
                    }
                    _ => {}
                }
    ...
    let zone_str = zone.unwrap_or_else(|| "local".to_string());

    // Generate unique registration function name based on type (not used currently)
    let _type_name = quote!(#impl_type).to_string().replace(" ", "");
    let _register_fn_name = syn::Ident::new(
        &format!("__register_scheduled_impl_{}", _type_name.replace("::", "_")),
